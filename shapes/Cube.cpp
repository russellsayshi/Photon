...
double Cube::intersectsWhen(Ray* ray) {
	...
	*/
	for(int i = 0; i < 6; i++) {	// eliminate impossible candidates
		Vec3 posVec = ray->vector->norm() * distances[i];
		Ray* toCenter = new Ray(posVec.x, posVec.y, posVec.z, center.x - posVec.x, center.y - posVec.y, center.z - posVec.z); //maybe switch center and position values? idk
		bool noShot = false;
